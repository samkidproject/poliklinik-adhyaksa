          ...
          {activeSubTab === 'master' && (
            <div className="bg-white/80 backdrop-blur-sm p-10 rounded-[3rem] border border-emerald-100 shadow-xl space-y-8">
              <div className="flex items-center gap-3 border-b border-emerald-50 pb-6">
                ...
                <h3 className="font-black uppercase text-sm text-emerald-900">Registrasi Pasien</h3>
              </div>
              <form onSubmit={registerPatient} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <button className="col-span-full py-5 bg-emerald-600 text-white rounded-[1.5rem] font-black uppercase text-xs shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all">Simpan Pasien</button>
              </form>
            </div>
          )}
          ...
          {activeSubTab === 'staff' && (
            <div className="bg-white/80 backdrop-blur-sm p-10 rounded-[3rem] border border-emerald-100 shadow-xl space-y-8">
              <div className="flex items-center gap-3 border-b border-emerald-50 pb-6">
                ...
                <h3 className="font-black uppercase text-sm text-emerald-900">Manajemen Petugas Poliklinik</h3>
              </div>
              <form onSubmit={saveStaff} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <button className="col-span-full py-5 bg-emerald-600 text-white rounded-[1.5rem] font-black uppercase text-xs shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all">Simpan Petugas</button>
              </form>
            </div>
          )}
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2">
              {activeSubTab === 'staff' ? (
                filteredStaffs.map((s: any) => (
                  <div key={s.id} className="p-4 bg-emerald-50/20 rounded-2xl border border-transparent flex items-center gap-4 group hover:bg-white hover:border-emerald-200 transition-all">
                ...
                filteredPatients.map((p: any) => (
                  <div key={p.nik} className="p-4 bg-emerald-50/20 rounded-2xl border border-transparent group hover:bg-white hover:border-emerald-200 transition-all">
...
function DokterView({ dbData, appId }: { dbData: any, user: any, appId: string }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between bg-white/80 backdrop-blur-sm p-6 rounded-[2rem] border border-emerald-100 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[9px] font-black text-emerald-400 uppercase tracking-[0.2em]">Bertugas Sebagai:</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {dbData.prescriptions.filter((p: any) => p.status !== 'Selesai').length > 0 ? (
            dbData.prescriptions.filter((p: any) => p.status !== 'Selesai').map((p: any) => (
              <div key={p.id} className="bg-white/80 backdrop-blur-sm p-8 rounded-[2.5rem] border border-emerald-100 shadow-xl space-y-4 hover:border-emerald-200 transition-all group">
                ...
                <div className="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100/50">
                  <p className="text-[9px] font-black text-emerald-400 uppercase mb-2">Resep Obat:</p>
                  <p className="text-sm font-bold leading-relaxed text-emerald-900">{p.diagnosis}</p>
                </div>
                <button onClick={() => finishPrescription(p.id)} className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase flex items-center justify-center gap-2 hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100">
